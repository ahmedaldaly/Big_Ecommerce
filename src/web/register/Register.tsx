...
const Register = () => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      setLoading(true)
      const res = await axios.post(`${BaseUrl}/api/vl/auth/register`, {
        name: data.name,
        email: data.email,
  ...
  return (
    <div className='w-[390px] h-auto flex  flex-wrap mx-auto mt-10'>
      ...
      <form onSubmit={onSubmit} className='my-5 w-full'>
        ...
        <input
          placeholder={Arabic ? 'ادخل الاسم' : 'Enter your name'}
          className='w-full px-5 mb-5 mt-2 border-2 border-gray-300 rounded-md h-10'
          {...register("name")}
        />
